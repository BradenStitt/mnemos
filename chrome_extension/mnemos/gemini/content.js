  ...
  function captureAndStoreMemory() {
    ...
    chrome.storage.sync.get(
      ...
      function (items) {
        ...
        if (items.memory_enabled === false) {
          return;
        }
        
        // Send memory to local API asynchronously without waiting for response
...
async function handleMem0Processing(capturedText, clickSendButton = false) {
  ...
  try {
    ...
    // No auth required for local service
    ...
    // No auth header needed for local requests
    ...
    const searchResponse = await fetch(
      "http://localhost:8000/api/cascading-retrieval?text_query=" + encodeURIComponent(message),
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    // Extract memories and their categories from local API response
    const memoryItems = (responseData.results || []).map((item, index) => {
      return {
        id: item._id || `memory-${Date.now()}-${index}`,
        text: item.chunk_text || item.text || item.content,
    ...
    // Store memory locally (non-blocking)
    fetch("http://localhost:8000/api/store-memory", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      ...
      .then((response) => {
        if (!response.ok) {
          console.error(`Failed to store memory locally: ${response.status}`);
        }
      })
      .catch((error) => {
...
function injectMem0Button() {
  ...
  async function tryAddButton() {
    ...
    if (!toolboxDrawer) {
      // Fallback: try to place button next to the send button
      const sendButton = getSendButton();
      if (sendButton) {
        // Avoid duplicate insertion
        if (document.querySelector('#mem0-gemini-fallback-btn')) {
          buttonRetryCount = 0;
          return;
        }
        ...
        wrapper.id = 'mem0-gemini-fallback-btn';
        ...
        // Create button
        const mem0Btn = document.createElement('button');
        mem0Btn.setAttribute('aria-label', 'Mem0');
        mem0Btn.style.cssText = `
          display: inline-flex;
          align-items: center;
          gap: 6px;
          padding: 6px 10px;
        ...
        mem0Btn.appendChild(iconImg);
        mem0Btn.appendChild(label);
        wrapper.appendChild(mem0Btn);
        wrapper.appendChild(notificationDot);

        // Simple hover tooltip
        wrapper.title = 'Add memories to your prompt';

        // Click handler
        mem0Btn.addEventListener('click', () => {
        ...
        if (sendButton.parentElement) {
          ...
          sendButton.insertAdjacentElement('beforebegin', wrapper);
        }
      ...
      // Neither toolbox nor send button found yet; retry
...
async function handleMem0Modal(sourceButtonId = null) {
  ...
  // No auth required for local service; proceed without popup
  ...
  try {
    ...
    const memoryItems = (responseData.results || []).map((item, index) => {
      return {
        id: item._id || `memory-${Date.now()}-${index}-${Math.random().toString(36).substring(2, 9)}`,
        text: item.chunk_text || item.text || item.content,
        categories: item.categories || []
      };
    });
    ...
    // Store memory locally asynchronously
    fetch("http://localhost:8000/api/store-memory", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
