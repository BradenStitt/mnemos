...
async function handleMem0Modal(popup, clickSendButton = false, sourceButtonId = null) {
  ...
  try {
    ...
    // No auth required for local service; proceed without login popup
    ...
    // Auth header not required for local calls
    ...
    // Store the current message asynchronously in local service (non-blocking)
...
function showLoginPopup() {
  // No-op for local Claude integration
}
...
function handleSyncClick() {
  ...
  getMemoryEnabledState().then(enabled => {
    ...
    chrome.storage.sync.get(["userLoggedIn", "access_token"], (data) => {
      // No login popup for local flow; allow sync regardless of auth state
      ...
      if (messages && messages.length > 0) {
        ...
        if (memoriesToSync.length > 0) {
          // Send memories to service (batch or individual, implementation retains existing function)
...
async function sendMemoriesToMem0(memories) {
  ...
  try {
    ...
    // Send all memories in one batch to Mem0 (same as ChatGPT integration)
