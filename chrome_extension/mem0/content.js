...
const SERVICE_CONFIG = {
  baseUrl: window.MEMORY_SERVICE_CONFIG?.SERVICE_BASE_URL || 'http://localhost:8000',
  searchEndpoint: window.MEMORY_SERVICE_CONFIG?.ENDPOINTS?.SEARCH || '/api/cascading-retrieval',
  storeEndpoint: window.MEMORY_SERVICE_CONFIG?.ENDPOINTS?.STORE || '/api/store-memory'
};
...
// Function to check if memory is enabled - simplified to always return true
function getMemoryEnabledState() {
  return Promise.resolve(true); // Always enabled, no auth needed
}
...
function addSendButtonListener() {
  ...
  function captureAndStoreMemory() {
    ...
    // Asynchronously store the memory to your service
    fetch(`${SERVICE_CONFIG.baseUrl}${SERVICE_CONFIG.storeEndpoint}`, {
      method: "POST",
      headers: {
...
// Handler for the modal approach - simplified without auth
async function handleMem0Modal(sourceButtonId = null) {
  ...
  try {
    // Search for relevant memories using your service
    const searchResponse = await fetch(
      `${SERVICE_CONFIG.baseUrl}${SERVICE_CONFIG.searchEndpoint}?text_query=${encodeURIComponent(message)}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!searchResponse.ok) {
      console.error('[Memory Service] API request failed with status:', searchResponse.status);
      throw new Error(
    ...
    // Extract memories from your service response
    ...
    // Store the current message asynchronously
    fetch(`${SERVICE_CONFIG.baseUrl}${SERVICE_CONFIG.storeEndpoint}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
// Function to show login popup - simplified to show service status
function showLoginPopup() {
  ...
  heading.textContent = 'Memory Service';
  ...
  message.textContent = 'Your memory service is running and ready to use!';
  ...
  statusButton.appendChild(logoDark);
  statusButton.appendChild(statusText);
  
  statusButton.addEventListener('mouseenter', () => {
    statusButton.style.backgroundColor = '#2d8f47';
  });
  
  statusButton.addEventListener('mouseleave', () => {
    statusButton.style.backgroundColor = '#34a853';
  });
